...
fetchIssueTimeline : Token -> IDSelector -> Task Error (List TimelineEvent)
...
-- NOTE: does not include pull requests!!!
            ...
            GB.extract <|
                ...
                <|
                    GB.extract
                        << GB.assume
                    <|
                        GB.inlineFragment (Just <| GB.onType "Issue")
...
decodeIssueState =
    let
        decodeToType string =
            case string of
                "IssueStateOpen" ->
                    Result.Ok IssueStateOpen

                "IssueStateClosed" ->
                    Result.Ok IssueStateClosed
...
encodeIssue record =
    JE.object
        ...
        , ( "labels", JE.list <| List.map encodeIssueLabel record.labels )
        , ( "cards", JE.list <| List.map encodeProjectCard record.cards )
        ]


encodeIssueLabel : IssueLabel -> JE.Value
encodeIssueLabel record =
    JE.object
        [ ( "name", JE.string record.name )

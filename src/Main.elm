...
engage config issue =
    Task.attempt (handleErr <| Engaged issue) <|
        Tracker.startStory
            config.trackerToken
...
disengage config issue =
    Task.attempt (handleErr <| Disengaged issue) <|
        Tracker.acceptStory
            config.trackerToken
            config.trackerProject
            issue.story.id
...
acceptOrphan config orphan =
    Task.attempt (handleErr <| always <| OrphanedStoryAccepted orphan) <|
...
deleteStory config orphan =
    Task.attempt (handleErr <| always <| OrphanedStoryDeleted orphan) <|
        Tracker.deleteStory
            config.trackerToken
            config.trackerProject
            orphan.story.id
...
storyClasses story =
    [ ( "chore", story.type_ == Tracker.StoryTypeChore )
    , ( "feature", story.type_ == Tracker.StoryTypeFeature )
    , ( "bug", story.type_ == Tracker.StoryTypeBug )
...
checkEngagement config issue =
    Task.attempt (handleErr <| IssueCommentsFetched issue) <|
...
fetchBacklog config =
    Task.attempt (handleErr BacklogFetched) <|
...
fetchStories config =
    Task.attempt (handleErr StoriesFetched) <|
...
fetchOrphanedStoryIssue model story =
    ...
    in
        case maybeRepo of
            Just repo ->
                Task.attempt (handleErr <| OrphanedStoryIssueFetched story) <|
                    GitHub.fetchIssue
            ...
            Nothing ->
                ...
                Task.attempt (handleErr OrphanedStoryIssueMissing) <|
                    Task.succeed story
            ...
            List.map
                (\x ->
                    case Dict.get x.id x2ys of
                        ...
                        Just ys ->
                            Dict.fromList (List.map (\y -> ( y.id, y )) ys)
                )
                xs
...
topicIndexInIteration { stories } topic =
    let
        ...
        indices =
            List.indexedMap
                (\i s ->
                    if topicIsForStory s topic then
                        i
                    else
                        max
                )
...
topicIsTriaged { stories } =
    List.any ((/=) Tracker.StoryTypeChore) (List.map .type_ stories)

...
var author$project$ForceGraph$DecodedNode = F7(
	function (id, x, y, vy, vx, size, value) {
		return {id: id, size: size, value: value, vx: vx, vy: vy, x: x, y: y};
	});
...
var author$project$Main$graphAllActivityCompare = F3(
	function (model, a, b) {
		...
		return A2(
			...
			latestActivity(a.nodes),
			latestActivity(b.nodes));
	});
var author$project$Main$graphImpactCompare = F3(
	function (model, a, b) {
		var _n0 = A2(
			elm$core$Basics$compare,
			elm$core$List$length(a.nodes),
			elm$core$List$length(b.nodes));
		if (_n0.$ === 'EQ') {
			var graphScore = A2(
				elm$core$List$foldl,
				F2(
...
var author$project$Main$graphUserActivityCompare = F4(
	function (model, login, a, b) {
		...
		return A2(
			elm$core$Basics$compare,
			latestUserActivity(a.nodes),
			latestUserActivity(b.nodes));
	});
...
var author$project$Main$graphId = function (graph) {
	return elm$core$String$fromInt(
		A3(
			...
			F2(
				function (node, acc) {
					return A2(elm$core$Basics$max, node.id, acc);
				}),
			0,
			graph.nodes));
};
...
var author$project$Main$cardRadiusWithFlair = F2(
	function (card, size) {
		...
		return (size + author$project$Main$flairRadiusBase) + highestFlair;
	});
...
var author$project$Main$viewNodeLowerUpper = F3(
	function (state, _n0, _n1) {
		...
		if (_n2.$ === 'Just') {
			var card = _n2.a;
			var radiiWithFlair = A2(author$project$Main$cardRadiusWithFlair, card, size);
			var pos = {x: x, y: y};
			...
			return _Utils_Tuple3(
				A2(
					...
					_Utils_Tuple2(
						value,
						A5(elm$svg$Svg$Lazy$lazy4, author$project$Main$viewCardFlair, card, size, pos, state)),
					fs),
				A2(
					elm$core$List$cons,
					_Utils_Tuple2(
						value,
						A5(elm$svg$Svg$Lazy$lazy4, author$project$Main$viewCardCircle, card, size, pos, state)),
					ns),
